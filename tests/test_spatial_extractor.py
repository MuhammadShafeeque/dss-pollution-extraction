...
def test_constructor_with_valid_coords(extractor):
    assert "x" in extractor.dataset.coords
    assert "y" in extractor.dataset.coords


def test_extract_points_from_geodata(extractor, point_geodata):
...
def test_extract_points_from_list(extractor):
...
def test_extract_polygons(extractor, polygon_geodata):
...
def test_spatial_subset_bbox(extractor):
